...
export type NexusInstance = {
    ...
    AddGenericPropertyGetter: (self: NexusInstance, Getter: (string, any) -> (any)) -> (),
...
function NexusInstance:__InitInternalProperties(): ()
    ...
    self:LockProperty("__GenericPropertyGetters")
...
function NexusInstance:__InitMetaMethods(): ()
    ...
    local GenericPropertyGetters = self.__GenericPropertyGetters
    ...
    Metatable.__index = function(_, Index: string): any
        ...
        local Value = OriginalIndexTable[Index]
        ...
        for _,Getter in GenericPropertyGetters do
            Value = Getter(Index, Value)
        end
...
These are called before any property-specific validators.
--]]
function NexusInstance:AddGenericPropertyGetter(Getter: (string, any) -> (any)): ()
    table.insert(self.__GenericPropertyGetters, Getter)
end
